#################################################################################################
# This script is used for ablation study for a distance of 500
# For analyzing the performance of other distances, change the following variables accordingly.
...
# Declare Variables
header=["Decision Threshold", "Model1", "Model2", "True Positive", "True Negative","False Positive", "False Negative", "Accuracy", "MCC", "ECE", "Brier_score", "NLL"]
...
# Specify File Paths
data_label_path_class = 'G:/ProjectData/ttnkoi_405-595_class/'
model_path_class = 'G:/ProjectData/ml_models_mvto_class_all/'

decision_points_list = ['405', '415', '425', '435', '445', '455', '465', '475', '485', '495', '505', '515', '525','535','545','555','565','575','585', '595']
# ===================== Functions =================================================================
...
def predict_proba(data_label_path_class,decision_point,reference_decision_point, model_file_suffix,data_file_suffix,label_file_suffix,model_path_class):
    """
...
for d in decision_points_list:
    ...
    data_test_y_class, predicted_proba_lower_1, predicted_proba_lower_both = predict_proba(data_label_path_class,decision_point,lower_point, model_file_suffix, data_file_suffix,label_file_suffix,model_path_class)
    data_test_y_class, predicted_proba_upper_1, predicted_proba_upper_both = predict_proba(data_label_path_class,decision_point,upper_point, model_file_suffix, data_file_suffix, label_file_suffix,model_path_class)
    ...
    predicted_class_proba_array = (distance_high*predicted_proba_lower_both + distance_low*predicted_proba_upper_both)/(distance_high+distance_low)
    ...
    print(str(decision_point) + ',' + str(lower_point) + ',' + str(upper_point) + ',' + str(tp) + ',' + str(tn) + ',' + str(fp) + ',' + str(fn) + ',' + str(mean_accuracy) + ',' + str(mcc) + ',' + str(ece) + ',' + str(brier_score) + ',' + str(nll))
